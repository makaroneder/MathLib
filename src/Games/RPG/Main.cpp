...
int main(void) {
    try {
        ...
        if (!playerImage.LoadFromPath(fs, path + "Player.tga")) MathLib::Panic("Failed to load image");
        MathLib::TGA oldManImage = MathLib::TGA(0, 0);
        if (!oldManImage.LoadFromPath(fs, path + "OldMan.tga")) MathLib::Panic("Failed to load image");
        MathLib::TGA blackPawnImage = MathLib::TGA(0, 0);
        if (!blackPawnImage.LoadFromPath(fs, path + "BlackPawn.tga")) MathLib::Panic("Failed to load image");
        ...
        while (true) {
            ...
            if (activeNPC == SIZE_MAX && currentShownQuest == SIZE_MAX) player.position += move * 15 * dt.GetValue();
            ...
            renderer.Fill(0);
            for (const Entity<MathLib::num_t>& entity : entities) entity.Draw(renderer);
            for (const NPC<MathLib::num_t>& npc : npcs) npc.Draw(renderer);
