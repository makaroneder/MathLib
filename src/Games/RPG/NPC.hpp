...
template <typename T>
struct NPC : Entity<T> {
    NPC(MathLib::Image& image, size_t health, const MathLib::Matrix<T>& position, const MathLib::Array<Dialog>& dialogs, const MathLib::Array<NPCTrigger>& triggers) : Entity<T>(image, health, position), dialog(0), dialogs(dialogs), triggers(triggers) {}
