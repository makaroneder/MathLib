...
namespace MathLib {
    ...
    String Integer::ToHexString(void) const {
        const String tmp = natural.ToHexString();
        return positive ? tmp : '-'_M + tmp;
    }
    bool Integer::Save(Writable& file) const {
        return natural.Save(file) && file.Write<bool>(positive);
    }
    bool Integer::Load(Readable& file) {
        return natural.Load(file) && file.Read<bool>(positive);
    }
    String Integer::ToString(const Sequence<char>& padding) const {
    ...
    bool Integer::LessThanEqual(const Integer& other) const {
