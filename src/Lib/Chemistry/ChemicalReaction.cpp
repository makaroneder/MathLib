        ...
        if (input.GetSize() == 2) {
            ...
            if ((input.At(0).IsHydroxide() && input.At(1).IsAcid()) || (input.At(1).IsHydroxide() && input.At(0).IsAcid())) {
                ...
                for (size_t i = 0; i < hydroxide.GetSize() - 2; i++)
                    if (!elements.Add(ChemicalElement(hydroxide.At(i), hydroxide.At(i).GetCount() * acid.At(0).GetCount()))) return Expected<ChemicalReaction>();
                for (size_t i = 1; i < acid.GetSize(); i++)
                    if (!elements.Add(ChemicalElement(acid.At(i), acid.At(i).GetCount() * hydroxide.At(hydroxide.GetSize() - 1).GetCount()))) return Expected<ChemicalReaction>();
                if (!ret.Add(ChemicalMolecule(elements, 1))) return Expected<ChemicalReaction>();
                Array<ChemicalElement> arr = Array<ChemicalElement>(2);
                arr.At(0) = Hydrogen(2);
                arr.At(1) = Oxygen(1);
                if (!ret.Add(ChemicalMolecule(arr, 1))) return Expected<ChemicalReaction>();
            }
        ...
        for (size_t i = 0; i < molecules.GetSize(); i++) {
            ...
            for (const ChemicalElement& element : chemElements) {
                ...
                if (!found) {
                    ...
                    rElement.coefficients.At(i, 0) = element.GetCount();
                    if (!elements.Add(rElement)) return Array<ChemicalReactionElement>();
                }
            }
        }
        return elements;
        ...
        for (size_t y = 0; y < l.GetSize(); y++) {
            for (const ChemicalReactionElement& elment : r) {
                if (elment.GetSymbol() == l.At(y).GetSymbol()) {
                    ...
                    if (!equations.AddEquation(AugmentedMatrix<num_t>(tmp, tmp.GetWidth() - 1))) return Expected<ChemicalReaction>();
                    break;
                }
            }
        }
