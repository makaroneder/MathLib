...
static constexpr num_t tau = 2 * M_PIl;
...
static constexpr num_t goldenAngle = M_PIl * (3 - std::sqrt(5));
static constexpr num_t magicAngle = std::atan(M_SQRT2l);
static constexpr num_t silverRatio = 1 + M_SQRT2l;
...
static constexpr num_t connectiveConstant = std::sqrt(2 + M_SQRT2l);
...
static constexpr num_t lemniscateConstant = M_PIl * gaussConstant;
...
static constexpr num_t ramanujanConstant = std::pow(M_El, M_PIl * std::sqrt(163));
...
static constexpr num_t gelfondConstant = std::pow(M_El, M_PIl);
static constexpr num_t secondFavardConstant = std::pow(M_PIl, 2) / 8;
static constexpr num_t gelfondSchneiderConstant = std::pow(2, M_SQRT2l);
...
static constexpr num_t firstNielsenRamanujanConstant = std::pow(M_PIl, 2) / 12;
...
static constexpr num_t levyConstant1 = std::pow(M_PIl, 2) / (12 * M_LN2l);
static constexpr num_t levyConstant2 = std::pow(M_El, levyConstant1);
...
static constexpr num_t vanDerPauwConstant = M_PIl / M_LN2l;
static constexpr num_t artinConstant = 0.37395581361920228805;
static constexpr num_t porterConstant = 1.46707807943397547289;
static constexpr num_t lochsConstant = (6 * M_LN2l * M_LN10l) / std::pow(M_PIl, 2);
...
static constexpr num_t feigenbaumConstant = 4.66920160910299067185;
static constexpr num_t chaitinConstant = 0.0078749969978123844;
static constexpr num_t robbinsConstant = (4 + 17 * M_SQRT2l - 6 * std::sqrt(3) - 7 * M_PIl) / 105 + Ln<num_t>(1 + M_SQRT2l) / 5 + (2 * Ln<num_t>(2 + std::sqrt(3))) / 5;
...
static constexpr num_t feigenbaumConstant = 2.50290787509589282228;
static constexpr num_t secondDuBoisReymondConstant = (std::exp(2) - 7) / 2;
static constexpr num_t erdosTenenbaumFordConstant = 1 - ((1 + Log<num_t>(10, Log<num_t>(2))) / Log<num_t>(10, 2));
