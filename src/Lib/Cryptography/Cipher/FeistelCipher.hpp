#ifndef MathLib_Cryptography_Cipher_FeistelCipher_H
#define MathLib_Cryptography_Cipher_FeistelCipher_H
#include "../Cipher.hpp"

namespace MathLib {
    struct FeistelCipher : Cipher {
        FeistelCipher(OneWayCipher& roundFunction);
        [[nodiscard]] virtual Array<uint8_t> Encrypt(const Sequence<uint8_t>& data, const CipherKey& key) const override;
        [[nodiscard]] virtual Array<uint8_t> DecryptPartial(const Sequence<uint8_t>& data, const CipherKey& key, const Interval<size_t>& range) const override;

        private:
        OneWayCipher& roundFunction;
    };
}

#endif