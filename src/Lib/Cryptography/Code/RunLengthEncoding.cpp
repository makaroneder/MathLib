...
namespace MathLib {
    Array<uint8_t> RunLengthEncoding::Encrypt(const Sequence<uint8_t>& data, const CipherKey& key) const {
        const size_t size = data.GetSize();
        if (key.type != CipherKey::Type::Normal || key.data.GetSize() != sizeof(bool) || !size) return Array<uint8_t>();
        const bool sizeFirst = key.data.AsT<bool>().Get();
        Array<uint8_t> ret;
    ...
    Array<uint8_t> RunLengthEncoding::DecryptPartial(const Sequence<uint8_t>& data, const CipherKey& key, const Interval<size_t>& range) const {
        const size_t size = data.GetSize();
        if (key.type != CipherKey::Type::Normal || key.data.GetSize() != sizeof(bool) || !size) return Array<uint8_t>();
        const bool sizeFirst = key.data.AsT<bool>().Get();
        Array<uint8_t> ret;
