...
namespace MathLib {
    ...
    bool BytePairEncodingSubstitute::Save(Writable& file) const {
        return file.Write<size_t>(name) && file.Write<size_t>(a) && file.Write<size_t>(b);
    }
    bool BytePairEncodingSubstitute::Load(Readable& file) {
        return file.Read<size_t>(name) && file.Read<size_t>(a) && file.Read<size_t>(b);
    }
    String BytePairEncodingSubstitute::ToString(const Sequence<char>& padding) const {
        return CollectionToString(padding) + Formatter<size_t>::ToString(name) + " = (" + Formatter<size_t>::ToString(a) + ", " + Formatter<size_t>::ToString(b) + ')';
    }
    bool BytePairEncodingSubstitute::operator==(const BytePairEncodingSubstitute& other) const {
        return name == other.name && a == other.a && b == other.b;
    }
    bool BytePairEncodingSubstitute::operator!=(const BytePairEncodingSubstitute& other) const {
        return !(*this == other);
    }
