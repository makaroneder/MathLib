...
namespace MathLib {
    ...
    Array<uint8_t> CBC::DecryptPartial(const Sequence<uint8_t>& data, const CipherKey& key, const Interval<size_t>& range) const {
        if (key.type != CipherKey::Type::MultiKey || key.children.GetSize() != 2) return Array<uint8_t>();
        ...
        Array<uint8_t> ret;
        for (size_t i = 0; i < size; i += blockSize) {
            const Array<uint8_t> tmp = prev;
            for (size_t j = 0; j < blockSize; j++) prev.AtUnsafe(j) = data.AtUnsafe(i + j);
            Array<uint8_t> decrypted = cipher.Decrypt(prev, key2);
            if (decrypted.GetSize() != blockSize) return Array<uint8_t>();
            for (size_t j = 0; j < blockSize; j++) decrypted.AtUnsafe(j) ^= tmp.AtUnsafe(j);
