...
namespace MathLib {
    OFB::OFB(Cipher& cipher) : cipher(cipher) {}
    Array<uint8_t> OFB::Encrypt(const Sequence<uint8_t>& data, const CipherKey& key) const {
    ...
    Array<uint8_t> OFB::DecryptPartial(const Sequence<uint8_t>& data, const CipherKey& key, const Interval<size_t>& range) const {
        if (key.type != CipherKey::Type::MultiKey || key.children.GetSize() != 2) return Array<uint8_t>();
