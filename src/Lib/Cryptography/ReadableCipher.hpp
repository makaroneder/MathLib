...
namespace MathLib {
    struct ReadableCipher : Cipher {
        [[nodiscard]] virtual Array<uint8_t> DecryptReadablePartial(Readable& readable, const CipherKey& key, const Interval<size_t>& range) const = 0;
        ...
        [[nodiscard]] Array<uint8_t> DecryptReadable(Readable& readable, const CipherKey& key) const;
    };
}
