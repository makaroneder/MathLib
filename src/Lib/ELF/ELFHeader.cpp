...
namespace MathLib {
    bool ELFHeader::IsValid(void) const {
        StartAndReturnFromBenchmark(*(const uint32_t*)signature == *(const uint32_t*)expectedSignature && version == expectedVersion && bitMode != BitMode::Invalid && endianness != Endianness::Invalid);
    }
    String ELFHeader::ToString(const Sequence<char>& padding) const {
        StartBenchmark
        String bitModeStr;
        switch (bitMode) {
