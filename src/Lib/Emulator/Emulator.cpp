...
size_t Emulator::ReadPositionedSizedBuffer(void* buffer, const size_t& size, const size_t& position) {
    size_t s = size;
...
size_t Emulator::WritePositionedSizedBuffer(const void* buffer, const size_t& size, const size_t& position) {
    size_t s = size;
    if (position + size > GetSize()) s = GetSize() - position;
