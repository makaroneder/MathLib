...
namespace MathLib {
    ...
    case op: {                                                                              \
        ...
        ModRMOperation({                                                                    \
            ...
            b = src->Get##size(false) + arg;                                                \
            result = a operation b;                                                         \
            if (!WritePositioned<uint##size##_t>(result, addr)) return false;               \
        })                                                                                  \
    ...
    case op: {                                                                          \
        ...
        ModRMOperation(switch (modrm.reg) {                                             \
            ...
            case 0b010: return WritePositioned<uint##size##_t>(~tmp.Get(), addr);       \
            case 0b011: return WritePositioned<uint##size##_t>(-tmp.Get(), addr);       \
            default: return false;                                                      \
        })                                                                              \
    }
