...
namespace MathLib {
    ...
    Node* TokenizeData(const Sequence<char>& str, size_t& i) {
        ...
        if (IsDigit(str.At(i))) {
            ...
            if (str.At(i) == 'i') {
                i++;
                return new Node(Node::Type::Constant, "0 + "_M + ret + 'i');
            }
            ...
            if (name == "i") return new Node(Node::Type::Constant, "0 + 1i"_M);
            return new Node(Node::Type::Variable, name);
        }
        else if (str.At(i) == '"') {
            i++;
            String ret = "";
            ...
            i++;
            return new Node(Node::Type::Array, ""_M, ret);
        }
        else if (str.At(i) == '[') {
            i++;
            SkipWhiteSpace(str, i);
            ...
            i++;
            return new Node(Node::Type::Absolute, ""_M, ret);
        }
        else if (str.At(i) == '-') {
            i++;
            Node* ret = TokenizeInternal(str, i);
            return ret ? new Node(Node::Type::Mul, ""_M, new Node(Node::Type::Constant, "-1"_M), ret) : nullptr;
        }
        else return nullptr;
    }
