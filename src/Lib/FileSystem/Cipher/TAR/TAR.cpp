...
namespace MathLib {
    bool TAR::IsValidHeader(Readable&, const Sequence<uint64_t>& key) const {
        return key.IsEmpty();
    }
    bool TAR::IsValidFooter(Readable& device, const Sequence<uint64_t>& key) const {
        if (!key.IsEmpty()) return false;
        for (uint8_t i = 0; i < 128; i++)
            if (device.Read<uint64_t>().GetOr(UINT64_MAX)) return false;
        return true;
    }
    bool TAR::Create(Writable& device, const Sequence<uint64_t>& key) const {
        if (!key.IsEmpty()) return false;
        for (uint8_t i = 0; i < 128; i++)
            if (!device.Write<uint64_t>(0)) return false;
        return true;
    }
    bool TAR::UpdateHeaderAndFooter(ByteDevice& device, const Sequence<uint64_t>& key) const {
        if (!key.IsEmpty()) return false;
        while (true) {
    ...
    FileCipherData TAR::Identify(Readable& device, const Sequence<uint64_t>& key, size_t& unpaddedSize) const {
    ...
    Array<uint8_t> TAR::Encrypt(const Sequence<uint8_t>& data, const Sequence<uint64_t>& key) const {
