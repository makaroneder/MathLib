...
size_t MemoryFS::OpenInternal(const String& path, const OpenMode& mode) {
...
bool MemoryFS::Close(const size_t& file) {
    if (files.GetSize() <= file || resources.GetSize() <= files.At(file).resource) return false;
    files.At(file).free = true;
    return true;
}
size_t MemoryFS::Read(const size_t& file, void* buffer, const size_t& size, const size_t& position) {
...
size_t MemoryFS::Write(const size_t& file, const void* buffer, const size_t& size, const size_t& position) {
...
size_t MemoryFS::GetSize(const size_t& file) {
    if (files.GetSize() <= file || resources.GetSize() <= files.At(file).resource) return 0;
