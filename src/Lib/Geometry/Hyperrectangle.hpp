        ...
        [[nodiscard]] virtual Array<Line<T>> ToLines(const Matrix<T>& rotation) const override {
            ...
            for (size_t i = 0; i < points.GetSize() - 1; i++) {
                for (size_t j = i + 1; j < points.GetSize(); j++) {
                    ...
                    if (diff == 1 && !ret.Add(Line<T>(RotateVector<T>(this->position + ConvertVectorDimension<T>(points.At(i), 3), this->position, rotation), RotateVector<T>(this->position + ConvertVectorDimension<T>(points.At(j), 3), this->position, rotation)))) ReturnFromBenchmark(Array<Line<T>>());
                }
            }
