...
namespace MathLib {
    ...
    bool Aseprite::Load(Readable& file) {
        AsepriteHeader header;
        if (!file.Read<AsepriteHeader>(header) || !header.IsValid() || header.bpp != 32) return false;
        frames = Array<Frame>(header.frames);
        for (uint32_t i = 0; i < header.frames; i++) {
            AsepriteFrameHeader frame;
            if (!file.Read<AsepriteFrameHeader>(frame) || !frame.IsValid()) return false;
            const uint32_t chunks = frame.chunks16 == UINT16_MAX && frame.chunks32 ? frame.chunks32 : frame.chunks16;
            for (uint64_t j = 0; j < chunks; j++) {
                uint32_t size = 0;
                if (!file.Read<uint32_t>(size) || size < sizeof(AsepriteChunkHeader)) return false;
                uint8_t buff[size];
                MemoryCopy(&size, buff, sizeof(uint32_t));
                if (!file.ReadBuffer(buff + sizeof(uint32_t), size - sizeof(uint32_t))) return false;
