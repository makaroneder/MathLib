...
namespace MathLib {
    ...
    bool P6::Save(Writable& file) const {
        ...
        if (!file.Puts("P6\n") || !file.Puts(ToString(width)) || !file.Write<char>(' ') || !file.Puts(ToString(height)) || !file.Puts(" 255\n")) return false;
    ...
    bool P6::Load(Readable& file) {
        ...
        for (size_t y = 0; y < height; y++) {
            for (size_t x = 0; x < width; x++) {
                uint8_t tmp[3] = { 0, 0, 0, };
                if (!file.ReadBuffer(tmp, SizeOfArray(tmp) * sizeof(uint8_t))) return false;
                pixels.AtUnsafe(x, y) = Color(tmp[0], tmp[1], tmp[2], UINT8_MAX).hex;
            }
        }
        return true;
    }
