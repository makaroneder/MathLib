#ifndef MathLib_Interfaces_Sequence_ByteArray_H
#define MathLib_Interfaces_Sequence_ByteArray_H
    ...
    struct ByteArray : Collection<uint8_t>, ByteDevice, Saveable, Printable {
        ...
        static ByteArray ToByteArray(const Sequence<T>& sequence) {
            ...
            for (size_t i = 0; i < size; i++) {
                const T tmp = sequence.AtUnsafe(i);
                const uint8_t* buff = (const uint8_t*)&tmp;
                for (size_t j = 0; j < sizeof(T); j++) ret.AtUnsafe(i * sizeof(T) + j) = buff[j];
            }
            return ret;
        }
        template <typename T>
