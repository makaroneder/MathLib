...
namespace MathLib {
    ...
    Ret InfiniteIntegralSubstitution(const void* func, T t) {
        const T tmp = 1 - t * t;
        return (*(const Function<Ret, T>*)func)(t / tmp) * (1 + t * t) / (tmp * tmp);
    }
    template <typename Ret, typename T>
    Ret Integrate(const Function<Ret, T>& func, const T& a, const T& b, size_t n, IntegrationType type) {
        if (IsInf(a) && IsInf(b)) return Integrate(FunctionPointer<Ret, T>((void*)&func, &InfiniteIntegralSubstitution<Ret, T>), -1 + eps, 1 - eps, n, type);
        switch (type) {
