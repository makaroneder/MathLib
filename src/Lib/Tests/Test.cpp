...
Test TestSelf(void) {
    ...
    Array<num_t> tmp = Array<num_t>(1);
    tmp.At(0) = 2; TestOperation(test, Matrix<num_t>(1, 1, tmp).Normalize().GetLength() == 1);
    tmp.At(0) = 0; TestOperation(test, !Matrix<num_t>(1, 1, tmp).Normalize().GetLength());
    tmp.At(0) = 0; TestOperation(test, !Matrix<num_t>(1, 1, tmp).GetInverse().HasValue());
    tmp.At(0) = 1; TestOperation(test, Matrix<num_t>(1, 1, tmp).GetInverse().HasValue());
