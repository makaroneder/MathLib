...
struct Regression : MathLib::Printable {
    Regression(const MathLib::RationalNumber& learningRate, const MathLib::Sequence<DataPoint>& dataSet, const MathLib::Sequence<MathLib::String>& coefficients, const MathLib::Function<Formula, const MathLib::Sequence<Formula>&, const Formula&>& function);
    [[nodiscard]] virtual MathLib::String ToString(const MathLib::Sequence<char>& padding = ""_M) const override;
    [[nodiscard]] bool Learn(void);
    [[nodiscard]] Formula Predict(const Formula& variable) const;

    private:
    MathLib::RationalNumber learningRate;
