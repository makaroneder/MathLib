...
int main(int, char**) {
    try {
        ...
        const CSV csv = CSV(fs.Open("src/TestPrograms/Stock/DVL.csv"_M, MathLib::OpenMode::Read).ReadUntil('\0')).Reduce(MathLib::MakeArray<MathLib::String>(
            "Low",
            "High"
        ));
        ...
        MathLib::Array<DataPoint> lowerBoundsTrainingSet;
        MathLib::Array<DataPoint> upperBoundsTrainingSet;
        for (size_t i = 0; i < size; i++) {
            const Formula tmp = Formula(RationalNumber(NaturalNumber::FromT<size_t>(i)));
            if (!lowerBoundsTrainingSet.Add(DataPoint(tmp, RationalNumber::FromFloatingPointString(csv.At("Low"_M, i))))) MathLib::Panic("Failed to add data point");
            if (!upperBoundsTrainingSet.Add(DataPoint(tmp, RationalNumber::FromFloatingPointString(csv.At("High"_M, i))))) MathLib::Panic("Failed to add data point");
        }
        ...
        const RationalNumber learningRate = RationalNumber(NaturalNumber::FromT<uint8_t>(1), NaturalNumber::FromT<uint8_t>(10), true);
