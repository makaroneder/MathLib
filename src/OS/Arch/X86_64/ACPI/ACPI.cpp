...
bool InitACPI(const RSDP* rsdp, bool nmi) {
    ...
    for (size_t i = 0; i < entries; i++) {
        ...
        if (*(const uint32_t*)signature == *(const uint32_t*)FADT::expectedSignature) {
            fadt = (FADT*)table;
        ...
        else if (*(const uint32_t*)signature == *(const uint32_t*)MADT::expectedSignature) {
            madt = (const MADT*)table;
            LogString("\tLocal APIC address: 0x"_M + MathLib::ToString(madt->localAPIC, 16) + '\n');
        }
        else if (*(const uint32_t*)signature == *(const uint32_t*)MCFG::expectedSignature) {
            mcfg = (const MCFG*)table;
            LogString("\tReserved: 0x"_M + MathLib::ToString(mcfg->reserved, 16) + '\n');
        }
        else if (*(const uint32_t*)signature == *(const uint32_t*)HPET::expectedSignature) {
        ...
        else if (*(const uint32_t*)signature == *(const uint32_t*)WAET::expectedSignature) {
        ...
        else if (*(const uint32_t*)signature == *(const uint32_t*)BGRT::expectedSignature) {
            const BGRT* bgrt = (const BGRT*)table;
            LogString("\tVersion: "_M + MathLib::ToString(bgrt->version, 10) + '\n');
            LogString("\tReserved: 0x"_M + MathLib::ToString(bgrt->reserved, 16) + '\n');
            LogString("\tOrientation: "_M + MathLib::ToString(90 * bgrt->orientation, 10) + '\n');
            LogString("\tDisplayed: "_M + MathLib::BoolToString(bgrt->displayed) + '\n');
            LogString("\tType: ");
            switch (bgrt->type) {
                case BGRT::Type::Bitmap: {
                    LogString("Bitmap");
                    break;
                }
                default: LogString("Unknown");
            }
            LogString(" (0x"_M + MathLib::ToString((uint8_t)bgrt->type, 16) + ")\n");
            LogString("\tAddress: 0x"_M + MathLib::ToString(bgrt->address, 16) + '\n');
            LogString("\tX: "_M + MathLib::ToString(bgrt->x, 10) + '\n');
            LogString("\tY: "_M + MathLib::ToString(bgrt->y, 10) + '\n');
        }
        LogString("}\n");
    }
    if (madt) {
