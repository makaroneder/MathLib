...
MathLib::Expected<uint64_t> GetMSR(uint32_t msr) {
    if (!HasMSR()) return MathLib::Expected<uint64_t>();
    uint32_t low = 0;
    uint32_t high = 0;
    asm volatile("rdmsr" : "=a"(low), "=d"(high) : "c"(msr));
    MathLib::Register tmp;
    tmp.Set32(low, false);
    tmp.Set32(high, true);
    return MathLib::Expected<uint64_t>(tmp.value);
}
bool SetMSR(uint32_t msr, uint64_t value) {
    if (!HasMSR()) return false;
    const MathLib::Register tmp = MathLib::Register(value);
