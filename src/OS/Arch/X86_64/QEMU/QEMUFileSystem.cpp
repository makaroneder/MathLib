...
size_t QEMUFileSystem::OpenInternal(const MathLib::Sequence<char>& path, MathLib::OpenMode mode) {
    ...
    uint32_t size = 0;
    if (!Foreach(MathLib::MakeFunctionT<bool, QEMUFileEntry>(nullptr, [&path, &selector, &size](const void*, QEMUFileEntry entry) -> bool {
...
size_t QEMUFileSystem::Write(size_t file, const void* buffer, size_t size, size_t position) {
    (void)file;
    (void)buffer;
    (void)size;
    (void)position;
...
MathLib::Array<MathLib::FileInfo> QEMUFileSystem::ReadDirectory(const MathLib::Sequence<char>& path) {
    if (!path.IsEmpty() && path != '/'_M) return MathLib::Array<MathLib::FileInfo>();
    MathLib::Array<MathLib::FileInfo> ret;
    return Foreach(MathLib::MakeFunctionT<bool, QEMUFileEntry>(nullptr, [&ret](const void*, QEMUFileEntry entry) -> bool {
        return !ret.Add(MathLib::FileInfo(MathLib::FileInfo::Type::File, MathLib::String(entry.name)));
    })) ? MathLib::Array<MathLib::FileInfo>() : ret;
}
