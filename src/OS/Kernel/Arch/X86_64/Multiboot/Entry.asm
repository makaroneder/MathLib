bits 32
section .multibootHeader
extern kernelStart
extern kernelEnd
extern bssStart

%define TextMode 1
%define FramebufferWidth 640 * !TextMode
%define FramebufferHeight 480 * !TextMode

Multiboot1Magic equ 0x1badb002
Multiboot1AlignModules equ 0
Multiboot1MemoryInfo equ 1
Multiboot1VideoInfo equ 2
Multiboot1AoutKludge equ 16
Multiboot1Flags equ 1 << Multiboot1VideoInfo | 1 << Multiboot1MemoryInfo | 1 << Multiboot1AlignModules
Multiboot2Magic equ 0xe85250d6
Multiboot2Length equ Multiboot2Header.end - Multiboot2Header
P2TableCount equ 512

align 4
Multiboot1Header:
    .magic: dd Multiboot1Magic
    .flags: dd Multiboot1Flags
    .checksum: dd -(Multiboot1Magic + Multiboot1Flags)
    .headerAddress: dd Multiboot1Header
    .loadAddress: dd kernelStart
    .loadEndAddress: dd bssStart
    .bssEndAddress: dd kernelEnd
    .entryAddress: dd Entry
    .framebufferMode: dd TextMode
    .framebufferWidth: dd FramebufferWidth
    .framebufferHeight: dd FramebufferHeight
    .framebufferDepth: dd 32 * !TextMode

align 8
Multiboot2Header:
    .magic: dd Multiboot2Magic
    .arch: dd 0
    .length: dd Multiboot2Length
    .checksum: dd -(Multiboot2Magic + Multiboot2Length)
    align 8
        .entryTagType: dw 3
        .entryTagFlags: dw 0
        .entryTagSize: dd 12
        .entryTagAddress: dd Entry
    align 8
        .flagsTagType: dw 4
        .flagsTagFlags: dw 0
        .flagsTagSize: dd 12
        .flagsTagConsoleFlags: dd 1 << 1
    %if !TextMode
    align 8
        .framebufferTagType: dw 5
        .framebufferTagFlags: dw 0
        .framebufferTagSize: dd 20
        .framebufferTagWidth: dd FramebufferWidth
        .framebufferTagHeight: dd FramebufferHeight
        .framebufferTagDepth: dd 32
    %endif
    align 8
        .moduleAlignmentTagType: dw 6
        .moduleAlignmentTagFlags: dw 0
        .moduleAlignmentTagSize: dd 8
    align 8
        .lastTagType: dw 0
        .lastTagFlags: dw 0
        .lastTagSize: dd 8
    .end:

section .bss
align 4096
P4Table: resb 4096
P3Table: resb 4096
P2Tables: resb 4096 * P2TableCount
align 16
Stack:
    .bottom: resb 2 * 4096
    .top:

section .data
%macro MakeGDTEntry 4
    .%1 equ $ - gdt
    .%1Limit: dw %2
    .%1Base1: dw 0
    .%1Base2: db 0
    .%1Access: db %3
    .%1Granularity: db %4
    .%1Base3: db 0
%endmacro

global gdt
gdt:
    MakeGDTEntry null, 0x0000, 00000000b, 00000000b
    MakeGDTEntry code64, 0xffff, 10011010b, 10100010b
    MakeGDTEntry data64, 0xffff, 10010010b, 11000000b
    MakeGDTEntry tss, 108, 10001001b, 00000000b
    .tssBase4: dd 0
    .tssReserved: dd 0
    .pointer:
        dw $ - gdt - 1
        dq gdt

section .text

; Input
;   eax = 0x2badb002 | 0x36d76289
;   ebx -> MB1Info | MB2Info
global Entry
Entry:
    cli
    mov esp, Stack.top
    mov edx, eax
    mov edi, bssStart
    mov ecx, kernelEnd
    sub ecx, edi
    xor al, al
    cld
    rep stosb
    mov esi, ebx
    mov edi, edx
    ; Check if CPUID is supported
    pushfd
    pop eax
    mov ecx, eax
    xor eax, 1 << 21
    push eax
    popfd
    pushfd
    pop eax
    push ecx
    popfd
    cmp eax, ecx
    je .error
    ; Check if long mode is supported
    mov eax, 0x80000000
    cpuid
    cmp eax, 0x80000001
    jb .error
    mov eax, 0x80000001
    cpuid
    test edx, 1 << 29
    jz .error
    ; Enable paging
    mov eax, P3Table
    or eax, 0b11
    mov [P4Table], eax
    %assign i 0
        %rep P2TableCount
            mov eax, P2Tables + 4096 * i
            or eax, 0b11
            mov [P3Table + 8 * i], eax
        %assign i i + 1
    %endrep
    xor ecx, ecx
    .mapP2:
        mov eax, ecx
        shl eax, 21
        or eax, 0b10000011
        %assign i 0
            %rep P2TableCount
                mov [P2Tables + 4096 * i + ecx * 8], eax
                add eax, 0x200000 * 512
            %assign i i + 1
        %endrep
        inc ecx
        cmp ecx, 512
        jne .mapP2
    mov eax, P4Table
    mov cr3, eax
    mov eax, cr4
    or eax, 1 << 5
    mov cr4, eax
    ; Enable long mode
    mov ecx, 0xc0000080
    rdmsr
    or eax, 1 << 8
    wrmsr
    mov eax, cr0
    or eax, 1 << 31
    mov cr0, eax
    lgdt [gdt.pointer]
    jmp gdt.code64:Entry64
    .error:
        mov al, 'E'
        out 0xe9, al
        mov al, 'r'
        out 0xe9, al
        out 0xe9, al
        mov al, 'o'
        out 0xe9, al
        mov al, 'r'
        out 0xe9, al
        jmp $

bits 64
extern Main

; Input
;   rsi -> MB1Info | MB2Info
;   rdi = 0x2badb002 | 0x36d76289
Entry64:
    mov ax, gdt.data64
    mov ds, ax
    mov es, ax
    mov fs, ax
    mov gs, ax
    mov ss, ax
    xor rbp, rbp
    jmp Main