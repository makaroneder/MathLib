...
bool InitMultiboot1(Multiboot1Info* info) {
    ...
    if (info->hasMemoryMap) {
        ...
        for (size_t i = entries; i; i--) {
            const Multiboot1MemoryMapEntry* entry = (const Multiboot1MemoryMapEntry*)((uintptr_t)info->memoryMapAddress + sizeof(Multiboot1MemoryMapEntry) * (i - 1));
            LogString("Memory map entry "_M + MathLib::ToString(i - 1) + ": {\n");
            ...
            LogString("}\n");
            memorySize = MathLib::Max<size_t>(memorySize, entry->address + entry->length);
            if (entry->type != MultibootMemoryMapEntryType::Available) continue;
            if (!AddMemoryRegion(entry->address, entry->length)) return false;
        }
    }
    return true;
}
