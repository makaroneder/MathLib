        ...
        switch (tag->type) {
            ...
            case Multiboot2Tag::Type::MemoryMap: {
                ...
                for (size_t i = entries; i; i--) {
                    const Multiboot2MemoryMapEntry* entry = (const Multiboot2MemoryMapEntry*)((uintptr_t)map->entries + map->entrySize * (i - 1));
                    LogString("\tEntry "_M + MathLib::ToString(i - 1) + ": {\n");
                    ...
                    if (!AddMemoryRegion(entry->address, entry->length)) return nullptr;
                }
                LogString("}\n");
                break;
            }
