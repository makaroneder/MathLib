...
MathLib::Date RTC::GetDate(void) const {
    while (IsUpdating()) {}
    MathLib::Date date;
    uint8_t century = 0;
    while (true) {
        uint8_t tmpCentury = 0;
        MathLib::Date tmp = MathLib::Date(Read(Register::RTCSeconds), Read(Register::RTCMinutes), Read(Register::RTCHours), Read(Register::RTCDay), Read(Register::RTCMonth), Read(Register::RTCYear));
    ...
    if (!(regB & 1 << 2)) {
        date = MathLib::Date(date.second % 16 + date.second / 16 * 10, date.minute % 16 + date.minute / 16 * 10, date.hour % 16 + date.hour / 16 * 10, date.day % 16 + date.day / 16 * 10, date.month % 16 + date.month / 16 * 10, date.year % 16 + date.year / 16 * 10);
    ...
    if ((uint8_t)centuryRegister) date.year += century * 100;
        ...
        date.year += MathLib::Date::currentYear / 100 * 100;
        if (date.year < MathLib::Date::currentYear) date.year += 100;
    }
    return date;
}
void RTC::SetDate(const MathLib::Date& date) {
    (void)date;
    // TODO:
}
