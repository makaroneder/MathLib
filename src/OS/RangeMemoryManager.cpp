...
bool RangeMemoryManager::AddRegion(Interval<uintptr_t> region) {
    if (!region.GetSize()) return false;
    if (!region.GetMin()) {
        if (region.GetSize() > 1) region = Interval<uintptr_t>(region.GetMin() + 1, region.GetMax());
        else return false;
    }
    return regions.Add(region);
}
void* RangeMemoryManager::At(size_t pos, size_t size) {
    const Interval<uintptr_t> kernelInterval = Interval<uintptr_t>((uintptr_t)&start, (uintptr_t)&end);
    for (const Interval<uintptr_t>& region : regions) {
...
size_t RangeMemoryManager::GetSize(void) const {
    size_t size = 0;
    for (const Interval<uintptr_t>& region : regions) size += region.GetSize();
    return size;
}