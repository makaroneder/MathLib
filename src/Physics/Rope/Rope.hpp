...
struct Rope : MathLib::LineShape<T> {
    Rope(const MathLib::Matrix<T>& gravity, const MathLib::Array<MathLib::Matrix<T>>& segments) : MathLib::LineShape<T>(MathLib::CreateVector<T>(0, 0, 0)), gravity(gravity), particles(MathLib::Array<MathLib::Particle<T>>(segments.GetSize())), lengths(MathLib::Array<T>(segments.GetSize() - 1)) {
